...
import { getCampaignControlCenter, getCampaigns } from "@/lib/api";
import { toCampaignControlSummary } from "@/lib/campaigns-data";

export default async function CampaignsPage() {
  const campaigns = await getCampaigns([]);
  const activeCampaign = campaigns[0] ?? null;
  const controlCenter = activeCampaign
    ? await getCampaignControlCenter(activeCampaign.id, null)
    : null;

  if (!activeCampaign || !controlCenter) {
    return (
      <main className="min-h-screen px-5 py-6 sm:px-8 lg:px-10">
        ...
        <section className="mt-6 border border-[var(--line)] bg-white p-6">
          ...
          <h1 className="mt-3 text-3xl font-semibold text-balance">No campaign audit feed</h1>
          <p className="mt-2 max-w-2xl text-pretty text-[var(--muted)]">
            Create a campaign through the API to populate the read-only operator summary.
          </p>
        </section>
      </main>
    );
