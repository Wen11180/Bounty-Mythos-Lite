...
export function ReportInspector({ actions, candidate, dossierPaths = [], markdownPath }: ReportInspectorProps) {
  return (
    <section aria-labelledby="report-inspector-title" className="border-t border-[var(--cc-border)] py-4">
      ...
      <dl className="mt-3 grid gap-3 text-xs"><div><dt className="text-[var(--cc-text-muted)]">就绪状态</dt><dd className="mt-1">{candidate?.reportReadiness.status ?? "等待候选审查"}</dd></div><div><dt className="text-[var(--cc-text-muted)]">下一步</dt><dd className="mt-1 leading-5">{candidate?.reportReadiness.nextAllowedAction ?? "完成证据与声明人工审查"}</dd></div>{markdownPath ? <div><dt className="text-[var(--cc-text-muted)]">Markdown draft</dt><dd className="mt-1 break-all font-mono">{markdownPath}</dd></div> : null}{dossierPaths.map((path) => <div key={path}><dt className="text-[var(--cc-text-muted)]">Mission dossier</dt><dd className="mt-1 break-all font-mono">{path}</dd></div>)}</dl>
