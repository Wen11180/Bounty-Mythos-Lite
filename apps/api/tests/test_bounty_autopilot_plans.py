...
def test_plan_binds_required_fields_and_digest():
    ...
    assert plan.cost_units == plan.max_requests
...
def test_plan_cost_units_are_server_calculated_from_request_limit():
    with pytest.raises(ValueError, match="plan_cost_units_server_calculated"):
        _plan(max_requests=5, cost_units=1)
