...
def test_wildcard_inclusion_excludes_the_apex_host():
    matcher = _matcher(include_hosts=("*.lab.local",))
    identity = parse_asset_url(
        "https://lab.local/api", provenance=AssetProvenance.DISCOVERED
    )

    record = decide_admission(identity, matcher)

    assert record.decision is AdmissionDecision.EXCLUDED
    assert record.reason == "host_not_included"


def test_path_prefix_does_not_match_a_sibling_with_the_same_text_prefix():
    matcher = _matcher(
        include_hosts=("lab.local",),
        include_path_prefixes=("/api",),
    )
    identity = parse_asset_url(
        "https://lab.local/api2", provenance=AssetProvenance.DISCOVERED
    )

    record = decide_admission(identity, matcher)

    assert record.decision is AdmissionDecision.EXCLUDED
    assert record.reason == "path_not_included"
