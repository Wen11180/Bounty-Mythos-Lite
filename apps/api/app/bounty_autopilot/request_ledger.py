...
class RequestReservation(StrictContract):
    ...
    method: str
    mutation_class: str
    # Alias only. Vault material, cookies, and authorization headers are never
    # representable in a durable reservation.
    ...
    @classmethod
    def require_safe_account_alias(cls, value: str | None) -> str | None:
        if value is not None and SAFE_ID_PATTERN.fullmatch(value) is None:
            raise ValueError("safe_account_alias_required")
        return value
