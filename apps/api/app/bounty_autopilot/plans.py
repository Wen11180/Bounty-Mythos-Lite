...
class ValidationPlan(StrictContract):
    ...
    def enforce_invariants(self) -> ValidationPlan:
        ...
        recipe = default_recipe_registry().get(
            self.recipe_ref.recipe_id,
            self.recipe_ref.version,
        )
        if recipe is None or recipe.ref != self.recipe_ref:
            raise ValueError("registered_recipe_ref_required")
        if self.mutation_inventory != recipe.mutation_inventory:
            raise ValueError("recipe_mutation_inventory_mismatch")
...
def build_validation_plan(**fields: Any) -> ValidationPlan:
    ...
    return ValidationPlan.model_validate_json(canonical_json_bytes(working))
