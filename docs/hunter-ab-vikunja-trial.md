...
Generated: 2026-07-12T13:11:44Z
...
| --- | --- | --- | --- | --- | --- | --- |
| my-gh-vikunja-task-authz-lab | my-gh-vikunja-task-authz-lab | refute | skipped_no_gold | ready | 0 | 3 |

## Decision quality (machine)

| case_id | expected | decision_quality | note |
| --- | --- | --- | --- |
| my-gh-vikunja-task-authz-lab | refute | pass | inspect evaluator notes |
...
- `H-001` → `refuted` root=`missing_object_ownership_check:delete_local_vikunja_task` duplicate_of=`None` evidence=['code:code.ts:owner_id_filter']
- `H-002` → `refuted` root=`missing_object_ownership_check:get_local_vikunja_task` duplicate_of=`None` evidence=['code:code.ts:owner_id_filter']
- `H-003` → `refuted` root=`missing_object_ownership_check:update_local_vikunja_task` duplicate_of=`None` evidence=['code:code.ts:owner_id_filter']
