...
- The orchestrator now has a bounded durable read-only research loop, but it is not a fully autonomous long-running research crew with externally executable specialist tool workflows.
...
- Agent execution is recorded and surfaced. Exploit Chain, Variant Analysis, and Deep Code Reasoning are independent persisted plan-only specialist tasks; broader specialist tool coverage remains incomplete.
...
- Attack-surface mapping now requires one completed same-snapshot observation and binds its checksummed redacted target-intake summary into the snapshot-bound target-model projection and runtime hypothesis target model. The projection also computes a deterministic, bounded attack-surface priority queue from traceable route, authorization, sink, object-identifier, and state-change signals; security-invariant and hypothesis stages consume the selected queue while retaining the complete fact digest for audit. Runtime inputs remain limited to traceable code, API, HAR, SARIF, and directly imported SBOM fact references; scanner correlation remains advisory and cannot independently create a candidate validation target.
...
- Snapshot-bound Exploit Chain reasoning runs as a plan-only workflow before candidate refutation, persisting normalized source references, primitives, preconditions, and refutation questions; missing or tampered plans block refutation.
- Variant Analysis and Deep Code Reasoning run as separate durable tasks after Exploit Chain. Their compact, snapshot-bound advisory projections retain no raw plan contents, bind their upstream projection digests and stage input lineage, and must match their own completed task and agent run before refutation or report review can consume them. Finding Dedup Risk persists after ranking and binds its candidate-input and ranking digests before it can appear in report review. These specialist projections never create candidates by themselves and keep all execution, validation, promotion, and submission flags false.
- An optional cross-source model advisory runs after hypothesis generation when a validated provider/model configuration is present. Its config digest, fact-pack digest, LLM audit, and completed-stage projection must match before candidate refutation can use it; it can only add bounded evidence and refutation hints to existing hypothesis IDs, never create a candidate or retain raw model content.
...
- Structured LLM workflows need broader model coverage, replayability, and calibrated evaluation beyond the current advisory stage.
...
6. Keep Exploit Chain, Variant Analysis, and Deep Code Reasoning schemas replayable and surface their independent plan-only provenance in review workflows.
