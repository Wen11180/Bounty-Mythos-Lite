...
最终目标不是普通安全 dashboard，也不是扫描器结果总结器，而是具备 Mythos / XBOW 风格真实自动发现高质量漏洞能力的本地 AI 漏洞研究系统。用户给定授权 scope、policy、API/HAR、授权本地代码和可选策略材料后，系统应能自动建模目标、选择高价值攻击面、生成漏洞假设、主动反证、去重、排序 Top 1-5 候选、整理证据需求、生成安全验证任务，并输出 submission-blocked 报告草稿交给人类审核。

当前阶段优先实现 A+B Autonomous Candidate Hunter：把授权 policy/scope/API/HAR 和授权本地代码结合起来，自动产出少量高质量漏洞候选，并为每个候选提供接口路径、代码路径、风险原因、反证问题、证据需求、安全验证计划、候选审核循环和 submission-blocked 报告草稿状态。

当前实现说明：仓库已经具备安全边界、artifact 摄入、候选生成、报告阻塞、候选审核计划和本地 Studio 工作台基础，但不能把现状描述成已经完成 Mythos/XBOW 级真实自动漏洞发现执行引擎。下一步核心目标是把 candidate-hunter review loop 升级为真正会循环推进的候选猎人：生成候选 -> 绑定证据 -> 反证 -> 去重 -> 排名 -> 安全验证任务 -> 报告草稿。
...
成熟形态下，系统应具备真实候选发现能力：不是被动罗列规则命中，而是能基于代码路径、API 行为、权限模型、业务不变量和历史学习信号，自主提出高价值漏洞候选并持续反驳弱候选。
